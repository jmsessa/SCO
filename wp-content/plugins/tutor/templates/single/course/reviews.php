...
<div class="tutor-pagination-wrapper-replaceable">
	<h3 class="tutor-fs-5 tutor-fw-bold tutor-color-black tutor-mb-24">
		<?php
			$review_title = apply_filters( 'tutor_course_reviews_section_title', 'avaliações e comentários dos Estudantes' );
			echo esc_html( $review_title, 'tutor' );
		?>
	</h3>

	<?php if(! is_array( $reviews ) || ! count( $reviews )): ?>
		<?php tutor_utils()->tutor_empty_state(__('Nenhuma revisão ainda', 'tutor')); ?>
	<?php else: ?>
			...
			<?php if($is_enrolled): ?>
				<button class="tutor-btn tutor-btn-primary write-course-review-link-btn">
					...
					<?php
						...
						$is_new ? _e('Escreva uma crítica', 'tutor') : _e('Editar a Critica', 'tutor');
					?>
				</button>
			<?php endif; ?>
		</div>
		...
		<div class="tutor-write-review-form" style="display: none;">
			<form method="post">
				<div class="tutor-star-rating-container">
					...
					<div class="tutor-form-group">
						<textarea name="review" placeholder="<?php _e('escreva uma crítica', 'tutor'); ?>"><?php echo stripslashes($my_rating ? $my_rating->comment_content : ''); ?></textarea>
					</div>
					<div class="tutor-form-group">
						<button type="submit" class="tutor_submit_review_btn tutor-btn tutor-btn-primary">
							<?php _e('Enviar revisão', 'tutor'); ?>
						</button>
					</div>
				</div>
			</form>
