...
if ($download->ID) {
	if(!is_user_logged_in()) {
		...
		$args = apply_filters( 'edd_purchase_link_defaults', array(
			'text'        => $button_behavior == 'direct' ? edd_get_option( 'buy_now_text', __( 'Comprar agora', 'easy-digital-downloads' ) ) : edd_get_option( 'add_to_cart_text', __( 'Purchase', 'easy-digital-downloads' ) ),
		...
		echo '<button class="'. esc_attr( $button_class ) .'">'.$button_text.'</button>';
	} else {
