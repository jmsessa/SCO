...
$profile_data = array(
	array( __( 'Data de registo', 'tutor' ), ( $rdate ? date_i18n( 'D d M Y, h:i:s a', $rdate ) : esc_html( '-' ) ) ),
	array( __( 'Nome Completo', 'tutor' ), ( $fname ? $fname : esc_html( '-' ) ) ),
	array( __( 'Alcunha', 'tutor' ), ( $lname ? $lname : __( '-' ) ) ),
	array( __( 'Nome de Utilizador', 'tutor' ), $uname ),
	array( __( 'E-mail', 'tutor' ), $email ),
	array( __( 'Telefone', 'tutor' ), ( $phone ? $phone : '-' ) ),
	array( __( 'Ocupação', 'tutor' ), ( $job ? $job : '-' ) ),
	array( __( 'Biografia', 'tutor' ), $bio ? $bio : '-' ),
);
?>

<div class="tutor-fs-5 tutor-fw-medium tutor-color-black tutor-mb-24"><?php _e( 'Meu Perfil', 'tutor' ); ?></div>
	...
	<?php foreach ( $profile_data as $key => $data ) : ?>
		<div class="tutor-row tutor-mb-24">
			...
			<div class="tutor-col-12 tutor-col-sm-7 tutor-col-lg-9">
				<?php echo $data[0] == 'Biografia' ? '<span class="tutor-fs-6 tutor-color-secondary">' . $data[1] . '</span>' : '<span class="tutor-fs-6 tutor-fw-medium tutor-color-black">' . $data[1] . '</span>'; ?>
			</div>
		</div>
	<?php endforeach; ?>
</div>
