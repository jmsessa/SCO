	...
	<div <?php tutor_post_class( 'tutor-wrap-parent tutor-full-width-student-profile tutor-page-wrap tutor-user-public-profile tutor-user-public-profile-' . $profile_layout . ' ' . $user_identifier ); ?> >
					...
					if ( $is_instructor ) {
						...
						?>
							<span>
								...
								<?php $course_count > 1 ? _e( 'Cursos', 'tutor' ) : _e( 'Curso', 'tutor' ); ?>
							</span>
							<span>
								<span>•</span>
							</span>
							<span>
								...
								<?php $student_count > 1 ? _e( 'Estudantes', 'tutor' ) : _e( 'Estudante', 'tutor' ); ?>
							</span>
						<?php
					} else {
						...
						?>
							...
							<?php $enrol_count > 1 ? _e( 'Cursos Matriculados', 'tutor' ) : _e( 'Cursos Matriculado', 'tutor' ); ?>
							</span>
							<span><span>•</span></span>
							<span>
								<span><?php echo $complete_count; ?></span> 
							<?php $complete_count > 1 ? _e( 'Cursos concluidos', 'tutor' ) : _e( 'Curso concluido', 'tutor' ); ?>
							</span>
							<?php
					}
					?>
		...
		<div class="tutor-container" style="overflow:auto">
			...
			<div class="tutor-user-profile-content tutor-d-block tutor-mt-72">
				<h3><?php _e( 'Biografia', 'tutor' ); ?></h3>
				<?php tutor_load_template( 'profile.bio' ); ?>
				
				<?php
				if ( $is_instructor ) {
					?>
						<h3><?php _e( 'Cursos', 'tutor' ); ?></h3>
						<?php
							add_filter(
								'courses_col_per_row',
								function() {
