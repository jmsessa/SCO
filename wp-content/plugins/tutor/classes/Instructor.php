...
class Instructor {
	...
	public function register_instructor() {
		...
		*/
		...
		if ( ! filter_var( tutor_utils()->input_old( 'email' ), FILTER_VALIDATE_EMAIL ) ) {
			$validation_errors['email'] = __( 'E-mail válido é obrigatório', 'tutor' );
		}
		if ( tutor_utils()->input_old( 'password' ) !== tutor_utils()->input_old( 'password_confirmation' ) ) {
			$validation_errors['password_confirmation'] = __( 'Suas senhas devem corresponder entre si. Por favor, cheque novamente.', 'tutor' );
		}
	...
	public function add_new_instructor() {
		...
		$required_fields = apply_filters(
			'tutor_instructor_registration_required_fields',
			array(
				'first_name'            => __( 'O campo do  nome é obrigatório', 'tutor' ),
				'last_name'             => __( 'O campo do alcunha é obrigatório', 'tutor' ),
				'email'                 => __( 'O campo Email é obrigatório', 'tutor' ),
				'user_login'            => __( 'O campo Nome do utilizador é obrigatório', 'tutor' ),
				'phone_number'          => __( 'O campo Número de telefone é obrigatório', 'tutor' ),
				'password'              => __( 'O campo palavra-passe é obrigatório', 'tutor' ),
				'password_confirmation' => __( 'Suas senhas devem corresponder entre si. Por favor, cheque novamente.', 'tutor' ),
			)
		);
		...
		if ( ! filter_var( tutor_utils()->input_old( 'email' ), FILTER_VALIDATE_EMAIL ) ) {
			$validation_errors['email'] = __( 'O campo Email é obrigatório', 'tutor' );
		}
		if ( tutor_utils()->input_old( 'password' ) !== tutor_utils()->input_old( 'password_confirmation' ) ) {
			$validation_errors['password_confirmation'] = __( 'Seus E-mail devem corresponder entre si. Por favor, cheque novamente.', 'tutor' );
		}
		...
		if ( ! is_wp_error( $user_id ) ) {
			...
			wp_send_json_success( array( 'msg' => __( 'O Formador foi adicionado com sucesso', 'tutor' ) ) );
		}
	...
	public function instructor_approval_action() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( array( 'message' => __( 'Acesso Negado', 'tutor' ) ) );
		}
